...
base_path = settings.FILE_ROOT_PATH
num_images = settings.IMAGENUM

p = Popen(["python", base_path + "Intelligence/GP_GPU/gp_cuda.py", data_path])


data_color = numpy.load(data_path + 'kernel-cl-'+str(num_images)+'.npy')
data = data_color
#data_texture = numpy.load(data_path + 'kernel-ht-'+str(num_images)+'.npy')
#data_shape = numpy.load(data_path + 'kernel-eh-'+str(num_images)+'.npy')
...
def start_search(request):
    ...
    objs = Image.objects.all()[:num_images]
    ...
    e = Experiment(
                ...
                goods = 0,
                satisfactories = 0,
                images_number_total = num_images,
...
def target_search(request):
    ...
    if "Night" in url:
        tags = pickle.load(open(data_path + "tag_to_img_" + str(num_images)))
...
def do_search(request, state = 'nostart'):
    ...
    if state == 'start':
        ...
        e.images_number_total = num_images
        ...
        if e.algorithm =='GP-SOM':
            global predictor
            predictor = GP_GPU.GP_GPU(e.images_number_iteration, e.images_number_total, firstround_images_shown, e.category)
    ...
    # Get a feedback vector using previouse_images from GP_GPU and state
    feedback = []
    #marks = []
...
def do_search2(request, state='notstart'):
    ...
    if state == 'start' :
        ...
        print(e.algorithm)
        if e.algorithm =='GP-SOM':
            predictor = GP_GPU.GP_GPU(e.images_number_iteration, e.images_number_total, e.category)
    ...
    # Get objects by indeces predicted by GP_GPU
